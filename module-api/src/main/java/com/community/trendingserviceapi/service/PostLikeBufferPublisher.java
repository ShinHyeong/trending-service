...
public class PostLikeBufferPublisher {
    private final SqsTemplate sqsTemplate;

    @Value("${sqs.queue.like-count:post-like-queue}")
    ...
    private final ConcurrentLinkedQueue<PostLikeEvent> eventQueue = new ConcurrentLinkedQueue<>();
    private final AtomicInteger queueSize = new AtomicInteger(0);

    public void enqueue(Long postId, int delta) {
        eventQueue.offer(new PostLikeEvent(postId, delta));
    ...
    public synchronized void flush() {
        ...
        PostLikeBatchMessage message = new PostLikeBatchMessage(
                batchId,
                serverInstanceId,
